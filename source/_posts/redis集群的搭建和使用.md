...
![](media/17041057699643.jpg)
在此服务器环境设置一个新key并查看：
![](media/17041057838269.jpg)
此时只在主服务器上设置了key，在从服务器应该自动复制过去一份，我们使用以下命令进入从服务器：
```bash
docker exec -it redis-slave bash
```
![](media/17041058036992.jpg)
...
```
![](media/17041059998154.jpg)
进入主服务器可以看到：
主服务器正确识别到两个slave：
![](media/17041060119123.jpg)
两个slave也正确的挂载到了master上：
![](media/17041060234617.jpg)
### 获取并修改sentinel配置
通过wget命令获取sentinel的配置文件：
```bash
wget http://download.redis.io/redis-stable/sentinel.conf
```
![](media/17041060500335.jpg)
修改配置文件以下几项
```conf
